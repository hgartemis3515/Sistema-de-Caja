...
class ClientController extends ControllerBase
{
	...
	public function importar_clientes($datapost) {
		...
		for ($indiceFila = 2; $indiceFila <= $numeroMayorDeFila; $indiceFila++) {
			...
			$num_documento = trim($hojaActual->getCell("C".$indiceFila)->getValue());
			$nombre_cliente = $hojaActual->getCell("D".$indiceFila)->getValue();
			$direccion = trim($hojaActual->getCell("E".$indiceFila)->getValue());
			$codigo_ubigeo = trim($hojaActual->getCell("F".$indiceFila)->getValue());
			$email = trim($hojaActual->getCell("G".$indiceFila)->getValue());
			$celular = trim($hojaActual->getCell("H".$indiceFila)->getValue());
			$telefono = trim($hojaActual->getCell("I".$indiceFila)->getValue());
			$num_cuenta_detraccion = trim($hojaActual->getCell("J".$indiceFila)->getValue());
			$detalle = trim($hojaActual->getCell("K".$indiceFila)->getValue());
