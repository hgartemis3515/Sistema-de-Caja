<?php 
Class Themefacturapdf {
	public function get_html_factura($contribuyente, $cliente, $documento, $detalle, $sunatmoneda, $ruta_qr, $modo, $ubigeo, $data_patrocinador, $html_cuentas_corrientes, $condicion_pago, $vendedor, $sucursal) {
		if($modo != 'prueba') {
		...
		foreach($detalle as $item) {
			...
			<tr style="border: none !important;">
				...
				<td class="cabecera_detalle_items" style="text-align: left;"><div style="width: 330px !important;">'.nl2br($item['DESCRIPCION_DET']).'</div></td>
				...
				<td class="cabecera_detalle_items" style="width: 80px;">'.$text_afectacion.'</td>
		...
		<tr>
			...
			<td class="texto_general text-right" style="padding: 0px !important;">
		...
		$resumen = $resumen.'
		<tr>
			<td class="texto_general" style="padding: 0px !important;">Total a Pagar:</td>
			<td class="texto_general text-right" style="padding: 0px !important;">
		...
		if($sucursal->leyenda_comprobantes != '') {
			$leyenda_comprobantes = '
			<tr style="border: none !important;">
				<td class="cabecera_detalle" style="text-align: left !important; font-size: 10px; " colspan="6">'.$sucursal->leyenda_comprobantes.'</td>
			</tr>
			';
		}
		...
		$direccion_fiscal = ($cliente->direccion_fiscal == '')?'-':($cliente->direccion_fiscal.' '.$ubigeo_ubicacion);
		...
		if(!empty($documento->nota)) {
			$nota_documento = nl2br($documento->nota);
		}
		...
		<body>    
				...
				<tbody>
					<tr>
						...
						<td class="cabecera_documento">
							...
							<p style="font-weight: 600;">FACTURA ELECTRÓNICA</p>
			...
			<table>
				<tbody>
					<tr>
						<td>
							<p>'.html_entity_decode($sucursal->txt_pdf_a4_1).'</p>
						</td>
					</tr>
				</tbody>
			</table>';
			...
			<div style="text-align:center; margin: 0px 0 2px 0;">
				<table style="margin: auto; border: 1px solid black; border-collapse: collapse;">
					<tbody>
						...
						<tr>
							<td class="cabecera_detalle_2">'.date("d-m-Y / H:i A", strtotime($fecha_doc_show)).'</td>
							...
							<td class="cabecera_detalle_2">'.$documento->nro_otr_comprobante.'</td>
						</tr>
					</tbody>
				</table>
			</div>
			
			<table cellpadding="0" cellspacing="0"  style="margin-top: 0px; border: 1px solid black; border-collapse: collapse;">
				<tbody>
					<tr>
						<td class="cabecera_detalle" style="width: 20px;">CANT.</td>
						<td class="cabecera_detalle">DESCRIPCIÓN</td>
						<td class="cabecera_detalle" style="width: 50px;">PRECIO</td>
						<td class="cabecera_detalle" style="width: 50px;">UNID/MED</td>
						<td class="cabecera_detalle" style="width: 50px;">AFECT.IGV</td>
						<td class="cabecera_detalle" style="width: 60px;">IMPORTE</td>
					</tr>
					'.$items_detalle_html.'
					<tr style="border: none !important;">
					...
					if(!empty($sucursal->txt_pdf_a4_2)) {
						$html = $html.'
						<tr style="border: none !important;">
							<td class="cabecera_detalle" style="text-align: left !important; font-size: 10px; " colspan="6">'.html_entity_decode($sucursal->txt_pdf_a4_2).'</td>
						</tr>';
					}
								...
								<table>
									<tbody>
										<tr>
											...
											<td style="width:280px;" class="texto_general">
												';
												if(!empty($nota_documento)) {
												$html = $html.'Observación: <br />
												<span style="font-size: 12px;">'.$nota_documento.'</span>
												<br><br>';
												}
												$html = $html.'
												...
												<br />
												<span style="font-size: 13px">Representación Impresa de la Factura Electrónica</span>
											</td>
										</tr>
										'.$aviso_texto_pruebas.'
									</tbody>
			...
			<table>
				<tbody>
					<tr>
						<td>
							<p>'.html_entity_decode($sucursal->txt_pdf_a4_3).'</p>
						</td>
					</tr>
				</tbody>
			</table>';
	...
	function get_html_factura_ticket($contribuyente, $cliente, $documento, $detalle, $sunatmoneda, $ruta_qr, $modo, $ubigeo, $data_patrocinador, $vendedor, $sucursal) {
		...
		<tr>
			<td align="right" class="texto_12">
				Total a Pagar: <span class="simbolo_moneda">'.$sunatmoneda->simbolo.'</span> '.custom_money_format($documento->total).'
			</td>
		</tr>
		';
		...
		if($documento->tipo_envio_sunat == 'prueba') {
			$aviso_texto_pruebas = '
			<tr>
				<td style="width:250px; margin-top: 15px !important; " colspan="2" class="texto_12">
					Representación Impresa de Documento Electrónico Generado En Una Versión de Pruebas. No tiene Validez!
				</td>
			</tr>
			';
		...
		$nota_documento = '';
		if(!empty($documento->nota)) {
			$nota_documento = '<strong>Observación: </strong><br />'.$documento->nota.'<br><br>';
		}
		...
		$direccion_fiscal = ($cliente->direccion_fiscal == '')?'-':ucwords(strtolower($cliente->direccion_fiscal.' '.$ubigeo_ubicacion));
		if(isset($documento->dir_destino) && !empty($documento->dir_destino)) {
			//$direccion_fiscal = $documento->dir_destino.' '.$ubigeo_ubicacion;
			$direccion_fiscal = $documento->dir_destino;
		}
		...
		<body> 
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<td align="center" class="texto_12">Fecha de Emisión: '.date("d-m-Y / H:i A", strtotime($fecha_doc_show)).'<td>
				</tr>
				<tr>
					<td align="center" class="texto_12">
				...
				<tr>
					<td align="center" class="texto_12">RUC: '.$cliente->num_doc.'<td>
				</tr>
				<tr>
			...
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td align="left"  class="texto_10">Importe en Letras: '.$documento->total_letras.' '.strtoupper($sunatmoneda->nombre).'<td>
				</tr>
				<tr>
				...
				$html = $html.'
				<tr>
					<td align="center"  class="texto_10">'.$nota_documento.'
