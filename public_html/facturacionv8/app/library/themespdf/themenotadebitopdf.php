<?php 
Class Themenotadebitopdf {
	public function get_html_notadebito($contribuyente, $cliente, $documento, $detalle, $sunatmoneda, $ruta_qr, $doc_modificado, $texto_motivo, $texto_doc_modificado, $modo, $ubigeo, $data_patrocinador, $html_cuentas_corrientes, $condicion_pago, $vendedor, $sucursal) {
		...
		if($documento->tipo_envio_sunat == 'prueba') {
			$aviso_texto_pruebas = '
			<tr>
				<td style="width:250px; font-family: Arial Narrow, Arial, sans-serif; font-size: 18px; color: red; font-weight: bold;" colspan="2">
		...
		if(!empty($documento->nota)) {
			$nota_documento = '<br /><strong>Observación: </strong>'.$documento->nota.'<br />';
		}
				...
				<tbody>
					<tr>
						...
						<td class="cabecera_documento">
							...
							<p style="font-weight: 600;">NOTA DE DÉBITO ELECTRÓNICA</p>
			...
			<div style="text-align:center; margin-top: 10px;">
				<table style="margin: auto; border: 1px solid black; border-collapse: collapse;">
					<tbody>
						...
						<tr>
							<td class="cabecera_detalle_2">'.date("d-m-Y", strtotime($fecha_doc_show)).'</td>
							...
							<td style="width:400px; vertical-align: top;">
								<table class="table">
									<tbody>
										...
										<tr>
											<td style="width:250px; font-family: Arial Narrow, Arial, sans-serif;" colspan="2">
												<strong>Motivo de Emisión:</strong> '.strtoupper($texto_motivo).'<br />
												<strong>Documento Relacionado:</strong> '.$texto_doc_modificado.' '.$doc_modificado->serie_comprobante.'-'.$doc_modificado->numero_comprobante.' | '.date("d-m-Y", strtotime($doc_modificado->fecha_comprobante)).'											
											</td>
										</tr>
										'.$aviso_texto_pruebas.'
									</tbody>
	...
	function get_html_notadebito_ticket($contribuyente, $cliente, $documento, $detalle, $sunatmoneda, $ruta_qr, $doc_modificado, $texto_motivo, $texto_doc_modificado, $modo, $ubigeo, $data_patrocinador, $vendedor, $sucursal) {
		if($modo != 'prueba') {
			$ruta_base = '/home/humbertoguadalup/public_html';
		} else {
			$ruta_base = 'https://facturalahoy.com';
		...
		foreach($detalle as $item) {
			...
			<tr class="detalletable">
				<td WIDTH="30">'.($item['CANTIDAD_DET'] + 0).'</td>
				<td WIDTH="70">'.$item['DESCRIPCION_DET'].'</td>
				<td WIDTH="45">'.$sunatmoneda->simbolo.' '.$precio_unitario_item.'</td>
				<td WIDTH="45">'.$sunatmoneda->simbolo.' '.custom_money_format($subtotal_item).'</td>
			</tr>
			';
		}
		...
		$resumen = $resumen.'
		<tr>
			<th>IGV ('.$documento->porcentaje_igv.'%):</th>
			<td style="text-align: right;">
				<span class="simbolo_moneda">'.$sunatmoneda->simbolo.'</span> '.custom_money_format($documento->total_igv).'
			</td>
		</tr>
				...
				<tbody>
					...
					<tr style="margin: 0px;">
						<td style="text-align: center; margin: 0px;" >
							<p style="font-size: 17px; line-height: 18px;">'.$contribuyente->nombre_comercial.' <br /> R.U.C.:'.$contribuyente->ruc.'</p>
							<p style="text-transform: none !important;">'.$sucursal->direccion.' '.$sucursal->urbanizacion.'<br /> '.$ubigeo_ubicacion.' <br />
								<img src="'.$ruta_base.'/facturacionv8/public/theme_doc_elect/images/telephone.png" style="width: 12px;" /> Telf.: '.$sucursal->telefono.'
								<br /><img src="'.$ruta_base.'/facturacionv8/public/theme_doc_elect/images/email.svg" style="width: 12px;" /> Email: '.$sucursal->email;
								...
								<strong style="font-size: 15px;">NOTA DE DÉBITO ELECTRÓNICA</strong><br /> 
								'.$documento->serie_comprobante.' - '.$this->zero_fill($documento->numero_comprobante, 6).'<br />
								<div style="font-size: 13px;">Fecha de Emisión: '.date("d-m-Y", strtotime($fecha_doc_show)).'<br />
								Señor (es): '.ucwords($cliente->razon_social).'<br />
								RUC N°: '.$cliente->num_doc.'<br />
								Direc.: '.ucwords(strtolower($cliente->direccion_fiscal)).'<br />
								------------------------------------------------------------------ 
								</div>
								<div>
								...
								<div style="font-size: 13px;">
									Importe en Letras: '.$documento->total_letras.'<br /><br />
									<img style="width: 120px; margin-bottom: 11px;" src="'.$ruta_qr.'" /><br />
