...
class DocumentoelectronicoController extends ControllerBase
{
	...
	*/
	...
	public function valida_datos_iniciales($usuario, $datapost) {
		...
		$factor_igv_sunat = isset($datapost['select_igv_sunat'])?floatval($datapost['select_igv_sunat']):$this->factor_igv_sunat;
		
		$valores_validos_factor_igv = $this->get_valores_validos_factor_igv();
		if (!in_array($factor_igv_sunat, $valores_validos_factor_igv)) {
			$resp['respuesta'] = 'error';
			$resp['titulo'] = 'Error';
			$resp['mensaje'] = 'El valor ingresado para el factor de IGV no es válido. Por favor ingresa un valor válido para el factor de IGV. Valores válidos: '.implode(', ', $valores_validos_factor_igv);
			return $resp;
		}

		$factor_igv_sunat = round(floatval($factor_igv_sunat/100), 3);
		...
		if(!empty($datapost['nro_placa_vehiculo'])) {
            if (strlen($datapost['transporte_nro_placa']) > 8) {
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
			...
			if(!preg_match('/^([A-Za-z0-9]+-?[A-Za-z0-9]+)$/', $datapost['transporte_nro_placa'])) {
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
	...
	public function getDocumentoElectronicoAction() {
        ...
        if($request->isAjax() == true) 
        {
			...
			foreach($items_detalle as $item) {
				...
				$factor_igv = round($item->factor_igv/100, 3);
	...
	public function get_secret_data($id_contribuyente) {
		...
		if($contribuyente->tipo_envio_sunat == 'produccion') {
			...
			if(isset($contribuyente->tipo_empresa_sunat) && $contribuyente->tipo_empresa_sunat == 'prico') {
				$secret_data['tipo_certificado'] = 'pse_ose';
