<?php
class ApinotacreditoController extends ControllerBase
{
	public function procesar_nota_credito($data, $token) {
		...
		$resp_cabecera = $this->validar_cabecera_notacredito($data);
		if(isset($resp_cabecera['respuesta']) && $resp_cabecera['respuesta'] == 'error') {
			return $resp_cabecera;
		}
		$data = $resp_cabecera['data'];
		$this->factor_igv_sunat = round($data['cabecera_comprobante']['porcentaje_igv']/100, 3);
	...
	public function validar_cabecera_notacredito($data) {
		...
		if(isset($cabecera['porcentaje_igv'])) {
			$cabecera['porcentaje_igv'] = floatval($cabecera['porcentaje_igv']) + 0;
			$valores_validos_factor_igv = $this->get_valores_validos_factor_igv();
			if(!in_array($cabecera['porcentaje_igv'], $valores_validos_factor_igv)) {
				$resp['respuesta'] = 'error';
				$resp['codigo'] = 'cabecera_comprobante';
				$resp['titulo'] = 'error';
				$resp['mensaje'] = 'Valor inválido para la propiedad cabecera_comprobante->porcentaje_igv, solo se aceptan los siguientes valores '.implode(', ', $valores_validos_factor_igv);
				return $resp;
			}
		} else {
	...
	public function generar_datapost($data) {
		...
		$array_tipo_operacion_validos = array('0101', '0200', '0201');
		...
		$datapost = array(
			...
			"id_motivo_nota_credito"			=> !isset($cabecera->id_motivo_nota_credito)?'':$cabecera->id_motivo_nota_credito, //validar
