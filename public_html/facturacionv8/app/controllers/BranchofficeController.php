...
class BranchofficeController extends ControllerBase
{
    ...
    public function insertAction(){
        ...
        if($request->isAjax() == true) 
        {
            ...
            $factor_igv = !isset($datapost['factor_igv_sucursal'])?18:floatval($datapost['factor_igv_sucursal']);
            $valores_validos_factor_igv = $this->get_valores_validos_factor_igv();
            if(!in_array($factor_igv, $valores_validos_factor_igv)) {
                $msj['respuesta'] = 'error';
                $msj['titulo'] = 'Error';
                $msj['mensaje'] = 'El valor ingresado para el factor de IGV no es válido. Por favor ingresa un valor válido para el factor de IGV. Valores válidos: '.implode(', ', $valores_validos_factor_igv);
