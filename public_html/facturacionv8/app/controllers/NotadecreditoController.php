<?php
class NotadecreditoController extends ControllerBase {
    
	public function procesar_notacredito($id_contribuyente, $idvendedor, $cabecera_inicial, $detalle, $emisor, $cliente, $datapost) {
		...
		$resp_guardado_bd = $this->guardar_notacredito_en_bd($id_contribuyente, $idvendedor, $cabecera_inicial, $detalle, $emisor, $cliente, $datapost);
		...
		if($modalidad_envio_sunat == 'no_enviar') {
			...
			$resp['mensaje'] = 'Se guardó correctamente el documento: <strong>'.$resp_guardado_bd['serie_comprobante'].'-'.$resp_guardado_bd['numero_comprobante'].'</strong>.<br /><strong>RECUERDA:</strong> El documento no ha sido enviado a SUNAT, por tanto usted debe enviarlo utilizando la pantalla de reporte de documentos!';
		...
		$resp_respuesta_api = $this->enviar_notacredito_api($resp_guardado_bd, $modalidad_envio_sunat);
		if($resp_respuesta_api['respuesta'] == 'error') {
			...
			$resp['mensaje'] = 'Se guardó correctamente el documento: '.$resp_guardado_bd['serie_comprobante'].'-'.$resp_guardado_bd['numero_comprobante'].'<span class="text-danger"> Sin embargo aún está "PENDIENTE DE ENVÍO A SUNAT".</span>';
    ...
    public function enviar_notacredito_api($data_doc_bd, $modalidad_envio_sunat) {
		...
		$data_anticipos = array();
		if($documento->id_cod_tipomotivo_credito == '01') { //anulación
			...
			if($documento_modificado) {
				...
				$data_anticipos = $factura->get_anticipos($documento_modificado);
			}
		} 
		...
		if($contribuyente->informar_condpago_sunat == 'si') {
			if($documento->tipo_venta == 'credito') {
				if($documento->monto_adeudado > 0) {
					...
					$forma_pago['detalle_cuotas'][]			= array(
																'fecha_vencimiento'	=> date("Y-m-d", strtotime($documento->fecha_pagopendiente)),
        ...
        $cabecera = array(
	        ...
	        'cod_tipo_motivo'           	=> $documento->id_cod_tipomotivo_credito,
	        'descripcion_motivo' 			=> $documento->descripcion_motivo_credito,
		...
		$data['emisor'] = $emisor;
		$data['data_anticipos'] = $data_anticipos;
		...
		$ruta_base_cpe_cdr = $data['secret_data']['ruta_dir_xml'].'nota_credito/';
		...
		$herramientas = new HerramientasController;
		$ruta = 'https://facturalahoy.com/api/notacredito';
		...
		if($modalidad_envio_sunat == 'solo_firma') {
			...
			try {
				...
				$resp['titulo'] = 'Error';
				$resp['error_code'] = 'envio_sunat';
				$resp['mensaje'] = 'El Documento N°: '.$serie_comprobante.'-'.$numero_comprobante.', se guardó correctamente. <br /> Sin embargo no se logró crear el documento electrónico correctamente, puedes intentarlo luego! ERROR: '.$e->getMessage();
				return $resp;
			}
		...
		if($resp_api->respuesta == 'error') {
			...
			if ($codio_error_sunat == 'soap-env:Client.1032') {
				...
				try {
					...
					$resp['titulo'] = 'Envío Correcto';
					$resp['error_code'] = 'envio_sunat';
					$resp['mensaje'] = 'El Documento N°: '.$serie_comprobante.'-'.$numero_comprobante.' ha sido Rechazado. <br /> Sin embargo no se logró guardar los datos de respuesta en el servidor local.<br /><br />RECOMENDACIÓN: Consultar el cdr en SUNAT. ERROR: '.$e->getMessage();
					return $resp;
				}

				if($documento->id_tipodoc_electronico == '07' || $documento->id_tipodoc_electronico == '08') {
					$data_doc_electronicoxnota['nota_estado_envio_sunat'] = $documento->estado_envio_sunat;
					$resp_doc_electronicoxnota = $this->guardar_registro_doc_electronicoxnota($data_doc_electronicoxnota);
		...
		if($resp_api->cod_sunat == '0' || $resp_api->cod_sunat == '2223') {
			...
			try {
				...
				$resp['titulo'] = 'Envío Correcto';
				$resp['error_code'] = 'envio_sunat';
				$resp['mensaje'] = 'El Documento N°: '.$serie_comprobante.'-'.$numero_comprobante.' fué enviado correctamente a SUNAT. <br /> Sin embargo no se logró guardar los datos de respuesta en el servidor local.<br /><br />RECOMENDACIÓN: Consultar el cdr en SUNAT. ERROR: '.$e->getMessage();
				return $resp;
			}
			...
			try {
				...
				$resp['titulo'] = 'Envío Correcto';
				$resp['error_code'] = 'envio_sunat';
				$resp['mensaje'] = 'El Documento N°: '.$serie_comprobante.'-'.$numero_comprobante.' fué enviado correctamente a SUNAT. <br /> Sin embargo no se logró guardar la ruta en la base de datos<br /><br />RECOMENDACIÓN: Consultar el cdr en SUNAT. ERROR: '.$e->getMessage();
				return $resp;
			}
			...
			try {
				...
				$resp['titulo'] = 'Envío Correcto';
				$resp['error_code'] = 'envio_sunat';
				$resp['mensaje'] = 'El Documento N°: '.$serie_comprobante.'-'.$numero_comprobante.' fué enviado correctamente a SUNAT. <br /> Sin embargo no se logró guardar la ruta del CDR en la base de datos<br /><br />RECOMENDACIÓN: Consultar el cdr en SUNAT. ERROR: '.$e->getMessage();
				return $resp;
			}
		...
		$resp['respuesta'] = 'error';
		$resp['titulo'] = 'Error';
		$resp['mensaje'] = !isset($resp_api->error_mensaje)?'':$resp_api->error_mensaje;
		return $resp;
	}

	public function validacion_de_nota_de_credito($id_contribuyente, $idvendedor, $cabecera_inicial, $detalle, $emisor, $cliente, $datapost) {
		$contribuyente = Contribuyente::findFirst(array("id_contribuyente = :id_contribuyente:", 'bind' => array('id_contribuyente' => $id_contribuyente)));
        ...
        $motivo_nota = SunatTiponotacredito::findFirst(array("id_tiponotacredito = :id_tiponotacredito:", 'bind' => array('id_tiponotacredito' => $datapost['id_motivo_nota_credito'])));
        if(!$motivo_nota) {
            $resp['respuesta'] = 'error';
            $resp['titulo'] = 'Error';
            $resp['mensaje'] = 'El motivo o tipo seleccionado para la presente nota de crédito no es válido!';
            return $resp;
        }

		$id_motivo = $motivo_nota->id_tiponotacredito;
        $descripcion_motivo = $motivo_nota->descripcion;

		$documento_modifica_array = explode(',', $datapost['serie_numero_doc_modificar']);

		if(!isset($documento_modifica_array[0]) || !isset($documento_modifica_array[1])) {
			$resp['respuesta'] = 'error';
			$resp['titulo'] = 'Error';
			$resp['mensaje'] = "La serie y/o número del documento a modificar es inválido";
			return $resp;
		}
		...
		if($verify_previus_note) {
			if($verify_previus_note->id_cod_tipomotivo_credito == '01') {
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
				$resp['mensaje'] = 'Previamente ya se registró la siguiente nota de crédito: '.$verify_previus_note->serie_comprobante.'-'.$verify_previus_note->numero_comprobante.' con Motivo 01:ANULACIÓN DE LA OPERACIÓN. Por tanto se ha bloqueado la creación de nuevas notas de crédito.';
				return $resp;
			}
		}
		...
		$resp['cabecera_inicial'] = $cabecera_inicial;

		//según SUNAT si la nota de crédito es con motivo 13: modificación de fecha de pago del comprobante.
		if($id_motivo == '13' || intval($id_motivo) == 13 || $id_motivo == '03' || intval($id_motivo) == 3) {

			if($id_motivo == '13' || intval($id_motivo) == 13) {
				if(!isset($datapost['opcion_tipo_venta']) && ($datapost['opcion_tipo_venta'] != 'credito' || $datapost['opcion_tipo_venta'] != 'on')) {
					//ya no se deben hacer validaciones de crédito porque anteriormente ya se validó todo.
					$resp['respuesta'] = 'error';
					$resp['titulo'] = 'Error';
					$resp['mensaje'] = 'Usted ha seleccionado el motivo con Código '.$id_motivo.': '.$descripcion_motivo.', por tanto al momento de crear la presente nota de crédito debe indicar la fecha de vencimiento de la cuota o cuotas, así como el monto adeudado.';
					return $resp;
				}
			}
			...
			foreach($detalle as $item) {
				$lista[] = array(
					...
					"UNIDAD_MEDIDA_DET"      	=> $item['UNIDAD_MEDIDA_DET'],
					"PRECIO_TIPO_CODIGO"     	=> $item['PRECIO_TIPO_CODIGO'], //Precio unitario (incluye el IGV) - catálogo N° 16
					"COD_TIPO_OPERACION_DET" 	=> $item['COD_TIPO_OPERACION_DET'], //id_tipoafectacionigv - CATALOGO No. 07
					"TEXT_TIPO_OPERACION_DET"	=> $item['TEXT_TIPO_OPERACION_DET'], //text_tipoafectacionigv - CATALOGO No. 07
					"CANTIDAD_DET"           	=> floatval($item['CANTIDAD_DET']) + 0,
					"PRECIO_DET"             	=> 0,
					"IGV_DET"                	=> 0,
					"ISC_DET"				 	=> 0,
					//"ICBPER_DET"				=> $icbper_det,
					"IMPORTE_DET"            	=> 0,
					"PRECIO_SIN_IGV_DET"  	 	=> 0,
					"CODIGO_DET"             	=> $item['CODIGO_DET'],
					"CODIGO_PRODUCTO"		 	=> $item['CODIGO_PRODUCTO'],
					"DESCRIPCION_DET"   	 	=> $item['DESCRIPCION_DET'],
					"IDPRODUCTO_DET"		 	=> $item['IDPRODUCTO_DET'],
					"ICBPER_DET"             	=> $item['ICBPER_DET'],
			...
			$datapost['txt_gravada_comprobante'] 		= 0;
			$datapost['txt_exonerada_comprobante'] 		= 0;
			$datapost['txt_inafecta_comprobante'] 		= 0;
			$datapost['txt_exportacion_comprobante'] 	= 0;
			$datapost['txt_icbper_comprobante'] 		= 0;
			$datapost['doc_impuesto_icbper'] 			= 0;
			$datapost['txt_total_comprobante'] 			= 0;
			$datapost['txt_igv_comprobante'] 			= 0;
			$datapost['txt_total_letras'] 				= '';
    ...
    public function guardar_notacredito_en_bd($id_contribuyente, $idvendedor, $cabecera_inicial, $detalle, $emisor, $cliente, $datapost) {
		$contribuyente = Contribuyente::findFirst(array("id_contribuyente = :id_contribuyente:", 'bind' => array('id_contribuyente' => $id_contribuyente)));
		...
		$resp_validacion_nc = $this->validacion_de_nota_de_credito($id_contribuyente, $idvendedor, $cabecera_inicial, $detalle, $emisor, $cliente, $datapost);
		if($resp_validacion_nc['respuesta'] == 'error') {
			echo json_encode($resp_validacion_nc);
			exit();
		}
		
		if(!isset($resp_validacion_nc['detalle'])) {
			$resp['respuesta'] = 'error';
            $resp['titulo'] = 'Error';
            $resp['mensaje'] = 'No se encuentra los items para la nota de crédito!';
            return $resp;
		}

		if(!is_array($resp_validacion_nc['detalle'])) {
			$resp['respuesta'] = 'error';
			$resp['codigo'] = 'detalle';
			$resp['titulo'] = 'error';
		...
		if(count($resp_validacion_nc['detalle']) <= 0) {
			$resp['respuesta'] = 'error';
			$resp['codigo'] = 'detalle';
			$resp['titulo'] = 'error';
		...
		$detalle = $resp_validacion_nc['detalle'];
		$datapost = $resp_validacion_nc['datapost'];
		...
		if($cabecera_inicial['modo_edicion'] == 'si') { 
			...
			$id_tipodoc_electronico_modifica = $datapost['tipo_docelectronico_modificar'];
			if(!isset($datapost['serie_numero_doc_modificar'])) {
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
				$resp['mensaje'] = "Debes Ingresar la Serie y Número del Documento a Modificar!";
				return $resp;
			}
			...
			*/
			...
			if($documento_para_modificar->porcentaje_igv != $factor_igv_sunat) {
				...
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
				$resp['mensaje'] = "El factor IGV utilizando en el documento: $serie_documento_modifica-$numero_documento_modifica es $documento_para_modificar->porcentaje_igv por tanto debe ser el mismo factor en la nota de crédito..";
				return $resp;
			}
			...
			if($id_tipodoc_electronico_modifica == '01') { //modifica una factura
				$serie_comprobante = $sucursal->notacredito_factura_serie;
			} else if ($id_tipodoc_electronico_modifica == '03') { //modifica una boleta
				$serie_comprobante = $sucursal->notacredito_boleta_serie;
			} else {
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
			...
			//DATOS PRINCIPALES COMO PRIMARY KEY PARA UNA NOTA DE CRÉDITO
        ...
        $new_documento->id_cod_tipomotivo_credito = $id_motivo;
        $new_documento->descripcion_motivo_credito = $descripcion;
		...
		foreach($detalle as $item) {
			...
			if(in_array($id_motivo, $array_motivos_modifican_stocks) && floatval($item["CANTIDAD_DET"]) > 0) {
				...
				//aquí aumentamos el stock para cada uno de los productos en el detalle!
				$modificar_stock = 'si';
				$modificar_stock = $this->get_sucursal_opcion($new_documento->id_contribuyente, $new_documento->id_sucursal, 'notacredito_modifica_stock');
					...
					if($unidad_medida) {
						if($unidad_medida->codigo != 'ZZ') {
							...
							$kardex_guardado = Kardex::findFirst(array("
								...
								id_compra 						is null
							", 'bind' => array(
								'docref_id_contribuyente' 		=> $new_documento->id_contribuyente, 
								'docref_id_tipodoc_electronico' =>  $new_documento->id_tipo_comprobante_modifica, 
								'docref_serie_comprobante' 		=> $new_documento->serie_documento_modifica, 
								'docref_numero_comprobante' 	=> $new_documento->nro_documento_modifica, 
								'docref_tipo_envio_sunat' 		=> $new_documento->tipo_envio_sunat, 
							...
							if($kardex_guardado) {
								...
								$data_kardex['detalle'] = 'Creación de Nota de Crédito';
		...
		$data_doc_electronicoxnota['cpe_id_contribuyente'] 			= $new_documento->id_contribuyente;
		$data_doc_electronicoxnota['cpe_id_tipodoc_electronico'] 	= $new_documento->id_tipo_comprobante_modifica;
		$data_doc_electronicoxnota['cpe_serie_comprobante'] 		= $new_documento->serie_documento_modifica;
		$data_doc_electronicoxnota['cpe_numero_comprobante'] 		= $new_documento->nro_documento_modifica;
		$data_doc_electronicoxnota['cpe_tipo_envio_sunat'] 			= $new_documento->tipo_envio_sunat;
		$data_doc_electronicoxnota['fecha_registro'] 				= date('Y-m-d H:i:s');
		$data_doc_electronicoxnota['nota_id_contribuyente']			= $new_documento->id_contribuyente;
		$data_doc_electronicoxnota['nota_id_tipodoc_electronico']	= $new_documento->id_tipodoc_electronico;
		$data_doc_electronicoxnota['nota_serie_comprobante'] 		= $new_documento->serie_comprobante;
		$data_doc_electronicoxnota['nota_numero_comprobante'] 		= $new_documento->numero_comprobante;
		$data_doc_electronicoxnota['nota_tipo_envio_sunat'] 		= $new_documento->tipo_envio_sunat;
		$data_doc_electronicoxnota['nota_estado_envio_sunat'] 		= $new_documento->estado_envio_sunat;
		$data_doc_electronicoxnota['nota_id_cod_tipomotivo_credito'] 	= $new_documento->id_cod_tipomotivo_credito;
		$data_doc_electronicoxnota['nota_id_cod_tipomotivo_debito'] 	= $new_documento->id_cod_tipomotivo_debito;
	...
	public function guardar_registro_doc_electronicoxnota($data) {
		$cpe_id_contribuyente 			= $data['cpe_id_contribuyente'];
		$cpe_id_tipodoc_electronico 	= $data['cpe_id_tipodoc_electronico'];
		$cpe_serie_comprobante 			= $data['cpe_serie_comprobante'];
		$cpe_numero_comprobante 		= $data['cpe_numero_comprobante'];
		$cpe_tipo_envio_sunat 			= $data['cpe_tipo_envio_sunat'];
		$fecha_registro 				= date('Y-m-d H:i:s');
		$nota_id_contribuyente 			= $data['nota_id_contribuyente'];
		$nota_id_tipodoc_electronico 	= $data['nota_id_tipodoc_electronico'];
		$nota_serie_comprobante 		= $data['nota_serie_comprobante'];
		$nota_numero_comprobante 		= $data['nota_numero_comprobante'];
		$nota_tipo_envio_sunat 			= $data['nota_tipo_envio_sunat'];
		$nota_estado_envio_sunat 		= $data['nota_estado_envio_sunat'];
		$nota_id_cod_tipomotivo_credito = $data['nota_id_cod_tipomotivo_credito'];
		$nota_id_cod_tipomotivo_debito 	= $data['nota_id_cod_tipomotivo_debito'];

		if($nota_id_tipodoc_electronico == '07') {
			$doc_electronicoxnota = DocElectronicoxnota::findFirst(
			array("
				[cpe_id_contribuyente] 			= :cpe_id_contribuyente: and 
				[cpe_id_tipodoc_electronico] 		= :cpe_id_tipodoc_electronico: and 
				[cpe_serie_comprobante] 			= :cpe_serie_comprobante: and 
				[cpe_numero_comprobante] 			= :cpe_numero_comprobante: and 
				[cpe_tipo_envio_sunat] 			= :cpe_tipo_envio_sunat: and 
				[nota_id_contribuyente] 			= :nota_id_contribuyente: and 
				[nota_id_tipodoc_electronico] 	= :nota_id_tipodoc_electronico: and 
				[nota_serie_comprobante] 			= :nota_serie_comprobante: and 
				[nota_numero_comprobante] 		= :nota_numero_comprobante: and 
				[nota_tipo_envio_sunat] 			= :nota_tipo_envio_sunat: and 
				[nota_id_cod_tipomotivo_credito] 	= :nota_id_cod_tipomotivo_credito:
			", 
			'bind' => array(
				'cpe_id_contribuyente' 				=> $cpe_id_contribuyente,
				'cpe_id_tipodoc_electronico' 		=> $cpe_id_tipodoc_electronico,
				'cpe_serie_comprobante' 			=> $cpe_serie_comprobante,
				'cpe_numero_comprobante' 			=> $cpe_numero_comprobante,
				'cpe_tipo_envio_sunat' 				=> $cpe_tipo_envio_sunat,
				'nota_id_contribuyente' 			=> $nota_id_contribuyente,
				'nota_id_tipodoc_electronico' 		=> $nota_id_tipodoc_electronico,
				'nota_serie_comprobante' 			=> $nota_serie_comprobante,
				'nota_numero_comprobante' 			=> $nota_numero_comprobante,
				'nota_tipo_envio_sunat' 			=> $nota_tipo_envio_sunat,
				'nota_id_cod_tipomotivo_credito' 	=> $nota_id_cod_tipomotivo_credito,
				)
			));
		} else if($nota_id_tipodoc_electronico == '08') {
			$doc_electronicoxnota = DocElectronicoxnota::findFirst(
				array("
					[cpe_id_contribuyente] 			= :cpe_id_contribuyente: and 
					[cpe_id_tipodoc_electronico] 		= :cpe_id_tipodoc_electronico: and 
					...
					[nota_numero_comprobante] 		= :nota_numero_comprobante: and 
					[nota_tipo_envio_sunat] 			= :nota_tipo_envio_sunat: and 
					[nota_id_cod_tipomotivo_debito] 	= :nota_id_cod_tipomotivo_debito:
				", 
				'bind' => array(
					'cpe_id_contribuyente' 			=> $cpe_id_contribuyente,
					'cpe_id_tipodoc_electronico' 	=> $cpe_id_tipodoc_electronico,
		...
		if(!$doc_electronicoxnota) {
			$doc_electronicoxnota = new DocElectronicoxnota();
			$doc_electronicoxnota->fecha_registro = $fecha_registro;
		}

		$doc_electronicoxnota->cpe_id_contribuyente 			= $cpe_id_contribuyente;
		$doc_electronicoxnota->cpe_id_tipodoc_electronico 		= $cpe_id_tipodoc_electronico;
		$doc_electronicoxnota->cpe_serie_comprobante 			= $cpe_serie_comprobante;
		$doc_electronicoxnota->cpe_numero_comprobante 			= $cpe_numero_comprobante;
		$doc_electronicoxnota->cpe_tipo_envio_sunat 			= $cpe_tipo_envio_sunat;
		$doc_electronicoxnota->nota_id_contribuyente 			= $nota_id_contribuyente;
		$doc_electronicoxnota->nota_id_tipodoc_electronico 		= $nota_id_tipodoc_electronico;
		$doc_electronicoxnota->nota_serie_comprobante 			= $nota_serie_comprobante;
		$doc_electronicoxnota->nota_numero_comprobante 			= $nota_numero_comprobante;
		$doc_electronicoxnota->nota_tipo_envio_sunat 			= $nota_tipo_envio_sunat;
		$doc_electronicoxnota->nota_estado_envio_sunat 			= $nota_estado_envio_sunat;
		$doc_electronicoxnota->nota_id_cod_tipomotivo_credito 	= $nota_id_cod_tipomotivo_credito;
		$doc_electronicoxnota->nota_id_cod_tipomotivo_debito 	= $nota_id_cod_tipomotivo_debito;

		try {
			if(!$doc_electronicoxnota->save()) {
				$msg = '';
				foreach ($doc_electronicoxnota->getMessages() as $message) {
					$msg = $msg.$message."</br>\n";
				}
				$resp['respuesta'] = 'error';
				$resp['titulo'] = 'Error';
				$resp['mensaje'] = 'Error al guardar docxnota: '.$msg;
				return $resp;
			}
		} catch (Exception $e) {
            $this->saveLogger($e);
			$resp['respuesta'] = 'error';
			$resp['titulo'] = 'Error';
			$resp['mensaje'] = 'Error al guardar docxnota: '.$e->getMessage();
			return $resp;
		}
